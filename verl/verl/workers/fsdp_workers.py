...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if not torch.distributed.is_initialized():
            ...
            # NOTE(dotv): 确保了每个 Worker 进程都正确地加入了分布式通信组
        ...
        if self.ulysses_sequence_parallel_size > 1:
            ...
            # NOTE(dotv): init_device_mesh 是 PyTorch 2.x 中用于 FSDP 和其他并行策略（如序列并行）的核心概念。它将物理设备（GPU）组织成一个逻辑网格，方便后续的并行操作。例如，一个 8-GPU 的节点，可以被看作是一个 [8] 的一维网格用于纯 FSDP，或者一个 [4, 2] 的二维网格用于 4 路数据并行 + 2 路序列并行。
        ...
        if self._is_actor:
            ...
            # micro bsz
            # 3. 对 ppo_micro_batch_size 进行归一化 (兼容旧配置)
            ...
            # 4. 对 ppo_micro_batch_size_per_gpu 进行断言检查
    ...
    def _build_model_optimizer(
        ...
        # 1. 加载 HF 模型配置和模型本体
        # get_init_weight_context_manager 使用了 meta device 初始化，避免在 CPU 上加载完整模型，节省内存
        ...
        # 4. 根据 PyTorch 版本选择 FSDP 实现 (旧版 FSDP vs. 新版 fsdp2 API)
        fsdp_strategy = self.config.actor.strategy
        if fsdp_strategy == "fsdp":  # 在 verl/trainer/config/actor/dp_actor.yaml
            # 使用 torch.distributed.fsdp.FullyShardedDataParallel 包装
            ...
            # 使用 torch.distributed.fsdp.fully_shard API
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            # NOTE(dotv, vllm): vLLMRollout 初始化
            ...
            # NOTE(dotv, vllm): 创建 Sharding Manager
            rollout_sharding_manager = FSDPVLLMShardingManager(
                module=self.actor_module_fsdp,
                inference_engine=rollout.inference_engine,  # <--- 把 rollout 实例传给了 sharding_manager
    ...
    def init_model(self):
        ...
        # 调用 _build_model_optimizer 来创建 Actor/Rollout 共享的模型
        ...
        if self._is_rollout:
            # NOTE(dotv, vllm): 创建 vLLMRollout 实例
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        timing_generate = {}
        # 1. 进入 Sharding Manager 的上下文
        with self.rollout_sharding_manager:
            log_gpu_memory_usage("After entering rollout sharding manager", logger=logger)

            # 2. Sharding Manager 的预处理
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)

            # 3. NOTE(dotv): 调用 vLLM 进行生成, verl/workers/sharding_manager/fsdp_vllm.py
            with simple_timer("generate_sequences", timing_generate):
                output = self.rollout.generate_sequences(prompts=prompts)

            log_gpu_memory_usage("After rollout generation", logger=logger)

            # 4. Sharding Manager 的后处理
