...
def compute_gae_advantage_return(
    ...
    with torch.no_grad():
        ...
        # GAE 中,
        # advantages (优势): 衡量的是在某个状态下，选择某个动作比平均水平（由 Value Model 评估）好多少。这个值是相对的，可正可负。Actor (策略模型) 的更新只依赖于 advantages。advantages 为正，就提高这个动作的概率；为负，就降低概率。
        # returns (回报): 是对未来奖励的折扣总和的估计。可以粗略理解为 Q-value 的一个样本。这个值是绝对的。Critic (价值模型) 的更新依赖于 returns。Critic 的目标是让自己的预测值 values 尽可能地接近 returns。
        advantages = verl_F.masked_whiten(advantages, response_mask)
    return advantages, returns

# NOTE(dotv): GRPO 的具体实现
# NOTE(sgm): 开发者 sgm 留下的一条重要说明：请注意，当前这个函数的实现只考虑了‘结果监督’（Outcome Supervision）的情况，也就是假定每个完整的回答只有一个标量（scalar）奖励值，而不是对每个步骤都有奖励。
...
def compute_grpo_outcome_advantage(
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    # 1. 将 token-level 的奖励求和，得到每个 response 的总分 (outcome reward)
    scores = token_level_rewards.sum(dim=-1)  # shape: (B, S)
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        # 3. 遍历所有样本，按 index (即 uid) 分组
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
        for idx in id2score:
            if len(id2score[idx]) == 1:
                # 如果组里只有一个样本，无法计算相对优势，设为0
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                # 论文中的标准 GRPO 公式：(r_i - mean(R)) / std(R)
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            else:
                # 不用标准差归一化的变体
                scores[i] = scores[i] - id2mean[index[i]]

        # 6. 将标量优势值广播到所有 response token 上
        scores = scores.unsqueeze(-1) * response_mask
    # 7. GRPO 中，returns 通常就等于 advantages
    # GRPO 的核心创新就是 干掉了 Critic (价值模型)。没有了 Critic 模型，我们也不再需要一个目标（returns）去监督它。Critic 的更新流程整个被移除了, 为什么返回两次 scores? 这是为了保持函数接口的一致性.
