...
class LLMEngine:
    ...
    def __init__(
        ...
        executor_class: Type[ExecutorBase],  # <--- 注意这个参数
        log_stats: bool,
        ...
        # NOTE(dotv): 核心调用在这里！LLMEngine 用来与底层计算 worker 通信的接口
    ...
    # NOTE(dotv): LLMEngine 并不硬编码它要使用哪个 ModelExecutor。它通过配置和工厂方法 (_get_executor_cls) 来动态决定。在 verl 的场景中，distributed_executor_backend 被设置为 "external_launcher"，所以 executor_class 最终会是 vllm.executor.uniproc_executor.ExecutorWithExternalLauncher。
    @classmethod
    ...
    def from_vllm_config(
        ...
        return cls(
            vllm_config=vllm_config,
            executor_class=cls._get_executor_cls(vllm_config),  # from_vllm_config 中是如何确定 executor_class 的
